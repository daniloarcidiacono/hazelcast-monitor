...
import {CardinalityEstimatorsProductDTO, CardinalityEstimatorSummaryDTO} from '@shared/dto/topic-products.dto';
...
@Component({
  ...
  styleUrls: [ './page-dashboard-cardinalityestimators.component.scss' ]
})
export class PageDashboardCardinalityEstimatorsComponent implements TabAwareComponent, OnDestroy {
  // Data subscription
  private dataSub: Subscription;

  // Current data
  private data: CardinalityEstimatorsProductDTO = undefined;
  ...
  public beforeShow(): void {
    if (!this.dataSub) {
      ...
      this.dataSub = this.hazelcastService.subscribeToCardinalityEstimators(this.clustersService.getCurrentCluster().instanceName, parameters).subscribe(
        (notice: SubscriptionNoticeResponseDTO<CardinalityEstimatorsProductDTO>) => {
          this.data = notice.notice;
        },
        (error: ErrorMessageDTO) => {
          this.snackbarService.show(`Could not fetch the cardinality estimators: ${error.errors}`);
        }
      );
    }
  }
